...
const Dashboard = () => {
  ...
  const handleStatusChange = async (id, status) => {
    ...
    ////Using optimistic updating  approach where UI is updated before side effects
    labData.map((patient) =>
  ...
  const handleDownload = async () => {
    try {
      ...
      const data = await fetch(
        "http://localhost:5000/api/v1/patients/download"
      );
  ...
  const filteredLabData = labData?.filter(
    (patients) => patients.status !== "completed"
  );
  const filteredPatientsData = patients?.filter(
    (patients) => patients.status !== "completed"
  );
    ...
    <div className=" flex flex-col overflow-scroll">
      ...
      {!isGeneralRole && (
        <div className="w-full flex justify-end px-10 py-5 ">
          {!isNurse ? (
            <FormDialog icon={<MdPersonAddAlt />} title="Add patient" />
          ) : (
